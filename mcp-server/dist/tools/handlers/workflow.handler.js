...
import { WorkflowRunner, getStateManager, AnalyzerAgent, GTMMCPAdapterImpl } from "gtm-agent-system";
// ==================== Internal MCP Call Function ====================
/**
 * 내부 MCP 호출 함수
 * Agent System이 MCP 도구를 호출할 때 사용
 */
function createInternalMCPCall(tagmanager) {
...
// ==================== Action Handlers ====================
async function handleAnalyze(tagmanager, args) {
    const { sessionId, sourceAccountId, sourceContainerId, sourceWorkspaceId, tagIds, includeAllDependencies = true } = args;
    ...
    analyzer.initialize({ sessionId, sourceWorkspace: sourceContext, targetWorkspace: sourceContext }, adapter);
    const response = await analyzer.execute({
        action: "analyzeTags",
        data: {
            tagIds: tagIdList,
...
async function handleReplicate(tagmanager, args) {
    const { sessionId, sourceAccountId, sourceContainerId, sourceWorkspaceId, targetAccountId, targetContainerId, targetWorkspaceId, tagIds, includeAllDependencies = true, skipNaming = false, skipValidation = false, dryRun = false, namePrefix, nameSuffix } = args;
    // Validate required fields
    if (!sourceAccountId || !sourceContainerId || !sourceWorkspaceId) {
        return createErrorResult("Missing required source workspace parameters");
    }
...
// ==================== Main Handler ====================
export const handleGtmWorkflow = async (tagmanager, args) => {

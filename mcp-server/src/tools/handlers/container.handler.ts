...
export const handleGtmContainer = async (
  ...
  switch (action) {
    ...
    case "remove": {
      throw new Error(
        `SAFETY: Container ${action} action is disabled. Creating, updating, or removing containers is not allowed via API for safety reasons. Please use GTM UI.`
      );
    }

    case "combine": {
      if (!containerId) throw new Error("containerId is required for combine action");
